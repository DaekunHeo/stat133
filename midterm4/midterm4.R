...
dice_sum <- function(k=2, B=100){
  dsum = vector()
  for(i in 1:B) {
    dsum[i] = sum(sample(1:6, k, replace = TRUE))
  }
  return(dsum)
}
...
text1 <- sapply(phrases, function(x) {
  grep("", phrases)
...
even <- paste0("even", seq(from= 2, by= 2, length.out= 1000))
...
hotelCal.split <- unlist(strsplit(x= tolower(gsub("[[:punct:]]", "", hotelCal)), " "))
...
updateDate <- function(dates, old.yr) {
  updated.dates = vector()
  sapply(dates, function(x) {
    old = as.numeric(unlist(strsplit(x, " "))[2])
    new.yr = as.numeric(old.yr)
    if(new.yr == old) {
      update = gsub(substr(length(unlist(strsplit(x, ""))) - 4, length(unlist(strsplit(x, ""))), test), "2015", x)
      updated.dates = c(updated.dates, update)
    }
  })
