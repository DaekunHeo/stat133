...
name <- "Daekun Heo"
github.name <- "DaekunHeo"
email.address <- "hdk777@berkeley.edu"
...
set.seed(42)
z <- sample(c(TRUE,FALSE), size= 5000, replace= TRUE, prob= c(.3,.7))
...
v <- c(1:100) 
for(i in 1:100) {
  v[i] = sample(1:6, size= 1, replace=TRUE)
}
...
cw4 <- sample(ChickWeight, 200, replace= TRUE)
...
plot(mtcars$disp, mtcars$wt, col= as.numeric(mtcars$am) + 1, 
     xlab= "Weight of the car in lb/1000", ylab= "Engine displacement in cu. in.", 
     main= "Engine Displacement Versus Weight")
...
plot(WorldBank$fertility.rate, WorldBank$life.expectancy, pch= ".", cex= 5,
     col= as.numeric(WorldBank$region) + 1, xlab = "Fertility Rate of Countries",
     ylab= "Life Expectancy of Countries")
abline(v= 2.1)
...
plot(WorldBank$fertility.rate[WorldBank$year == "1960"], 
     WorldBank$life.expectancy[WorldBank$year == "1960"], pch= ".", cex = 5, 
     col= as.numeric(WorldBank$region) + 1, xlab = "Fertility Rate of Countries",
     ylab= "Life Expectancy of Countries", main= "1960")
abline(v= 2.1)

plot(WorldBank$fertility.rate[WorldBank$year == "2013"], 
     WorldBank$life.expectancy[WorldBank$year == "2013"], pch= ".", cex = 5, 
     col= as.numeric(WorldBank$region) + 1, xlab = "Fertility Rate of Countries",
     ylab= "Life Expectancy of Countries", main= "2013")
abline(v= 2.1)
...
hist(WorldBank$GDP.per.capita.Current.USD[WorldBank$lending == "IDA"], breaks= 250,
     xlab= "GDP Per Capita in Current USD",main= "GDP of Countries with IDA Lending Status")
...
# and the function [abs()] for absolute value
 max.diff.rain <- sapply(rain, function(x) abs(max(diff(x))))
...
prop.rain <- sapply(rain, function(x) length(x[x>0])/length(x))
...
#    mean(cases)-mean(controls) for one permutation
 ...
 PermDiff <- function(cases, controls, k=5000){
   ...
   for(i in 1:k) {
     ...
     save.cases[i]= sample(total, k)
     save.controls[i]= total[-save.cases]
     output[i]= mean(save.cases) - mean(save.controls)
   }
...
NumJackpot <- function(k, B){
  ...
  check = vector()
  for(i in 1:B) {
    winner= sample(1:19, 3)
    jack= 0
    for(j in 1:k) {
      players= sample(1:19, 3)
      if(players[1] == winner[1] & players[2] == winner[2] & players[3] == winner[3]) {
        jack= jack + 1
      }
    }
    n.jackpot[i]= jack
...
for(i in 1:length(k)) {
  ...
  hist(store[[i], freq= FALSE, xlab= "Number of Winners")
}
...
# Create a vector [match1] with the index for all matches to "A*T" or "G*T"
 match1 <- sort(c(grep(".+A*T.+", dna), grep(".+G*T.+", dna)))

# [2 pts]
# Create a vector [dna2] where you have removed all entries whose length is not a multiple of 3
check= sapply(dna, function(x) {
  haha= unlist(strsplit(x, ""))
  return(length(haha)%%3 == 0)
}) 
dna2 <- dna[check]
